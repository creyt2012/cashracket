...
class WithdrawRepo {
  ...
  Future<bool> withdrawRequestInfo(String methodId, String currencyId, String coinAMount, String account) async {
    ...
    String? token = await DataBase().retrieveString('token');
    var response =
        await http.post(url, headers: {'Authorization': 'Bearer $token'}, body: <String, String>{'method_id': methodId, 'currency_convert_id': currencyId, 'coins': coinAMount, 'account': account});
    if (response.statusCode == 200) {
      return true;
    } else {
